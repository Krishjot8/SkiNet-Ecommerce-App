...
export class CheckoutPaymentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    loadStripe(
      'pk_test_51My3TlBM8uSaVWEYDngL3z5dvXkBEKxCxfSlUhbBWh9MQur6fIh1ZarCdZCbKSbdTAy72PGuDLIjNKQotnF1P3Np00hXq4z06P'
    ).then((stripe) => {
      this.stripe = stripe;
  ...
  async submitOrder() {
    ...
    try {
      ...
      if (paymentResult.paymentIntent) {
        this.basketService.deleteBasket(basket); //after the order it will reset and delete the current basket
  ...
  private async createdOrder(basket: Basket | null) {
    if (!basket) throw new Error('Basket is null');
